...
| 9.09 | "Open the review on launch" off, put an unfinished task on yesterday with `sql`, `restart`, `screen` | The home page opens, not the review; the status line counts `1 in review` in red; `M` opens the review. On again, clear the gate (`delete from meta where key like 'review_%'`, since the review opens itself once a working day, DOMAIN.md 13) and `restart`: the review opens. |
