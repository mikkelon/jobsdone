...
| 9.25 | `scripts/install --keybind "SUPER + T" --tiled` with a fake `HOME`/`XDG_CONFIG_HOME` and a scratch `bindings.lua` holding the old `-- jobsdone: begin` block | The old block is gone; a keybind block binds `SUPER + T`; the window block is bare markers; the modmask check reports a taken `SUPER + T` correctly when `hyprctl` is faked to say so. `scripts/uninstall` removes both blocks. On the clean VM, which has no Rust: `make install` refuses with the sentence that names the pacman command; after that command, `make install` (answer the keybind prompt, or run `scripts/install --keybind`), `uat/vm keys super-shift-j`, `hyprctl clients -j` shows class `org.omarchy.jobsdone` floating at 870x650. Run by hand in the `uat/vm ssh` shell, the same install says `SUPER + SHIFT + N is already bound to 'Editor'` and asks before overwriting it; after a `y` the notes block holds `hl.unbind("SUPER + SHIFT + N")` above its `o.bind`, `hyprctl binds -j` lists one bind on those keys, "Jobsdone notes", and `uat/vm keys super-shift-n` opens the same floating window on the notes page. After an `n`, and with no terminal attached, the keys stay Omarchy's. `scripts/uninstall` gives them back. |
