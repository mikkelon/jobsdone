...
`note archive ID` takes a note out of the list without deleting it and
...
If the CLI archives a note open in the TUI, the editor keeps saving into
it; the note leaves the list when the editor is closed. If the CLI deletes a note with unsaved text in the TUI, that text is also saved
